...
MAYA_ROOT=$(dirname "${BASH_SOURCE}")/..
source "${MAYA_ROOT}/hack/lib/init.sh"

maya::golang::verify_go_version
...
failure_file="${MAYA_ROOT}/hack/.golint_failures"
...
all_packages=(
	$(go list -e ./... | egrep -v "/(third_party|vendor|staging/src/k8s.io/client-go/pkg|generated|clientset_generated)" | sed -e 's|^code.xxxxx.cn/platform/galaxy/||' -e "s|^_${MAYA_ROOT}/\?||")
)
failing_packages=(
	$(cat $failure_file)
)
