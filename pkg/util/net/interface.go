...
// RouteFile struct of route file
...
func isInterfaceUp(intf *net.Interface) bool {
	...
	if intf.Flags&net.FlagUp != 0 {
		alog.V(6).Infof("Interface %v is up", intf.Name)
		return true
	}
	return false
}
...
func getMatchingGlobalIP(addrs []net.Addr, family AddressFamily) (net.IP, error) {
	if len(addrs) > 0 {
		for i := range addrs {
			alog.V(6).Infof("Checking addr  %s.", addrs[i].String())
			...
			if memberOf(ip, family) {
				if ip.IsGlobalUnicast() {
					alog.V(6).Infof("IP found %v", ip)
					return ip, nil
				}
				alog.V(6).Infof("Non-global unicast address found %v", ip)
			} else {
				alog.V(6).Infof("%v is not an IPv%d address", ip, int(family))
			}
...
func getIPFromInterface(intfName string, forFamily AddressFamily, nw networkInterfacer) (net.IP, error) {
	...
	if isInterfaceUp(intf) {
		...
		alog.V(6).Infof("Interface %q has %d addresses :%v.", intfName, len(addrs), addrs)
		matchingIP, err := getMatchingGlobalIP(addrs, forFamily)
		if err != nil {
			return nil, err
		}
		if matchingIP != nil {
			alog.V(6).Infof("Found valid IPv%d address %v for interface %q.", int(forFamily), matchingIP, intfName)
...
func chooseIPFromHostInterfaces(nw networkInterfacer) (net.IP, error) {
	...
	for _, family := range []AddressFamily{familyIPv4, familyIPv6} {
		alog.V(6).Infof("Looking for system interface with a global IPv%d address", uint(family))
		for _, intf := range intfs {
			if !isInterfaceUp(&intf) {
				alog.V(6).Infof("Skipping: down interface %q", intf.Name)
				continue
			}
			if isLoopbackOrPointToPoint(&intf) {
				alog.V(6).Infof("Skipping: LB or P2P interface %q", intf.Name)
				continue
			}
			addrs, err := nw.Addrs(&intf)
			if err != nil {
			...
			if len(addrs) == 0 {
				alog.V(6).Infof("Skipping: no addresses on interface %q", intf.Name)
				continue
			}
			for _, addr := range addrs {
				...
				if !memberOf(ip, family) {
					alog.V(6).Infof("Skipping: no address family match for %q on interface %q.", ip, intf.Name)
					continue
				}
				// TODO: Decide if should open up to allow IPv6 LLAs in future.
				if !ip.IsGlobalUnicast() {
					alog.V(6).Infof("Skipping: non-global address %q on interface %q.", ip, intf.Name)
					continue
				}
				alog.V(6).Infof("Found global unicast address %q on interface %q.", ip, intf.Name)
				return ip, nil
			}
		}
	}
...
func chooseHostInterfaceFromRoute(routes []Route, nw networkInterfacer) (net.IP, error) {
	for _, family := range []AddressFamily{familyIPv4, familyIPv6} {
		alog.V(6).Infof("Looking for default routes with IPv%d addresses", uint(family))
		for _, route := range routes {
			if route.Family != family {
				continue
			}
			alog.V(6).Infof("Default route transits interface %q", route.Interface)
			finalIP, err := getIPFromInterface(route.Interface, family, nw)
			if err != nil {
				return nil, err
			}
			if finalIP != nil {
				alog.V(6).Infof("Found active IP %v ", finalIP)
				return finalIP, nil
			}
		}
	}
	alog.V(6).Infof("No active IP found by looking at default routes")
	return nil, fmt.Errorf("unable to select an IP from default routes")
}

// ChooseBindAddress choose a bind address ip
